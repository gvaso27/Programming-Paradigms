...
static int StringHash(const void *e, int numBuckets){
  int i;
  unsigned long hashcode = 0;
  char *ch = *(char **)e;
  for (i = 0; i < strlen(ch); i++)
    hashcode = hashcode * kHashMultiplier + tolower(ch[i]);
...
vector staties_urls_list;
hashset stop_words_list;
hashset root;
...
static int word_cmp(const void *elem1, const void *elem2) {
    const words *word1 = (const words *)elem1;
    const words *word2 = (const words *)elem2;
...
static int elem_num_cmp(const void *elem1, const void *elem2) {
    const vector_elem *ve1 = (const vector_elem *)elem1;
    const vector_elem *ve2 = (const vector_elem *)elem2;
    
    if (ve1->num == ve2->num) return 0;
    return (ve1->num < ve2->num) ? 1 : -1;
}
...
static void vector_elem_free(void *elem) {
    if (!elem) return;
    vector_elem *vec = (vector_elem *)elem;
    if (vec->url) free(vec->url);
    if (vec->title) free(vec->title);
}

static void word_free(void *elem) {
    if (!elem) return;
    words *word = (words *)elem;
    if (word->str) free(word->str);
    if (word->vect) {
        VectorDispose(word->vect);
        free(word->vect);
    }
}

static void stopword_hasshet_init() {
    const int BUCKET_COUNT = 1009;
    HashSetNew(&stop_words_list, sizeof(char *), BUCKET_COUNT, StringHash, str_cmp, str_free);
    FILE *stopwords_file = fopen("data/stop-words.txt", "r");
    if (stopwords_file) {
      streamtokenizer token_reader;
      STNew(&token_reader, stopwords_file, kNewLineDelimiters, true);
      const size_t BUFFER_SIZE = 200;
      char buffer[BUFFER_SIZE];
      while (STNextToken(&token_reader, buffer, BUFFER_SIZE)) {
          char *word_copy = strdup(buffer);
          if (word_copy) {
              HashSetEnter(&stop_words_list, &word_copy);
          }
      }
      STDispose(&token_reader);
      fclose(stopwords_file);
    } else {
      fprintf(stderr, "Error: Could not open stop-words.txt\n");
      return;
    }
...
int main(int argc, char **argv) {
  ...
  stopword_hasshet_init();
  HashSetNew(&root, sizeof(words), 1009, StringHash, word_cmp, word_free);
  VectorNew(&staties_urls_list, sizeof(char *), str_free, 1009);
  ...
  HashSetDispose(&stop_words_list);
  HashSetDispose(&root);
  VectorDispose(&staties_urls_list);
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        ...
        if (HashSetLookup(&stop_words_list, &dupWord) == NULL) {
            words newWord;
            newWord.vect = malloc(sizeof(vector));
            newWord.str = strdup(dupWord);
            VectorNew(newWord.vect, sizeof(vector_elem), vector_elem_free, 47);
            words *existingWord = HashSetLookup(&root, &newWord);
            vector_elem newElem;
            newElem.num = 1;
            newElem.url = strdup(articleURL);
            newElem.title = strdup(articleTitle);
            if (existingWord == NULL) {
                VectorAppend(newWord.vect, &newElem);
                HashSetEnter(&root, &newWord);
            } else {
                int idx = VectorSearch(existingWord->vect, &newElem, str_cmp, 0, false);
                if (idx == -1) {
                    VectorAppend(existingWord->vect, &newElem);
                    word_free(&newWord);
                } else {
                    vector_elem *foundElem = VectorNth(existingWord->vect, idx);
                    foundElem->num++;
                    VectorSort(existingWord->vect, elem_num_cmp);
                    word_free(&newWord);
                }
            }
        }
...
static void ProcessResponse(const char *word) {
  if (WordIsWellFormed(word)) {
char *search_term = strdup(word);
    if (!search_term) {
        fprintf(stderr, "Error: Memory allocation failed\n");
        return;
    }
    if (HashSetLookup(&stop_words_list, &search_term)) {
    ...
    VectorNew(&search_vector, sizeof(vector_elem), vector_elem_free, 199);
    words *results = HashSetLookup(&root, &search_entry);
    free(search_entry.str);
    vector_elem_free(&search_vector);
    if (!results) {
        printf("None of today's news articles contain the word \"%s\".\n", search_term);
        free(search_term);
        return;
    }
    const int result_count = VectorLength(results->vect);
    for (int i = 0; i < result_count; i++) {
        vector_elem *article = (vector_elem *)VectorNth(results->vect, i);
        printf("%d.) \"%s\" [search term occurs %s%d time%s]\n",
               i + 1,
               article->title,
               article->num == 1 ? "" : "",
               article->num,
