...
(define (intersection-points circles)
  (define (intersect-with-circle circle rest-circles)
    (apply append (map (lambda (c) (intersect circle c)) rest-circles)))
  (if (or (null? circles) (null? (cdr circles)))
      '()
      (append (intersect-with-circle (car circles) (cdr circles))
              (intersection-points (cdr circles)))))

;; 2. distance-product
(define (distance-product pt pt-list)
  (apply * (map (lambda (other-pt)
                  (if (equal? pt other-pt) 1
                      (dist pt other-pt)))
                pt-list)))
...
(define (rate-points points)
  (map (lambda (pt)
         (list (distance-product pt points) pt))
       points))
...
(define (quicksort lst cmp)
  (if (null? lst)
      ...
      (let ((pivot (car lst))
            (rest (cdr lst)))
        (append (quicksort (filter (lambda (x) (cmp x pivot)) rest) cmp)
                (list pivot)
                (quicksort (filter (lambda (x) (not (cmp x pivot))) rest) cmp)))))

(define (sort-points rpt-list)
  (quicksort rpt-list (lambda (a b) (< (car a) (car b)))))

;; 5. clumped-points
(define (clumped-points pt-list)
  (map cadr
       (prefix-of-list (sort-points (rate-points pt-list))
                       (quotient (length pt-list) 2))))
...
(define (average-point points)
  (let* ((avg-pt (list (average (map car points))
         ...
         (distance (distance-product avg-pt points)))
    (list distance avg-pt)))
