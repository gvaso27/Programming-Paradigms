      ...
      if(previouslySeenFilms.find(films[i]) == previouslySeenFilms.end()){
        ...
        for(int j = 0; j < cast.size(); j++){
          if(previouslySeenActors.find(cast[j]) == previouslySeenActors.end()){
            ...
            path clonePath = curr;
            clonePath.addConnection(films[i], cast[j]);
            if(targetActor == cast[j]){
              cout<<clonePath;
              return;
            }
            partialPaths.push_back(clonePath);
          }
        }
      }
    }
  }
  cout<<"No path between those two people could be found."<<endl<<endl;
}
