...
static void reccursion(map<string, Definition>& grammar, string word){

  Production prod = grammar[word].getRandomProduction();

   for(Production::iterator i = prod.begin(); i < prod.end(); i++){

    string currWord = *i;
    
    if(currWord[0] == '<'){
      reccursion(grammar, currWord);
    }else{
      if(currWord != "."){
        cout<<" "<<currWord;
      }else{
